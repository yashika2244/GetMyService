...
const GetProfileFromSearch = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-50 to-purple-100 py-10 px-4 flex flex-col items-center">
      <button
        onClick={() => navigate(-1)}
        className="self-start mb-8 text-sm text-purple-700 hover:underline flex items-center gap-1"
      >
        <span>&larr;</span> Back
      </button>
      ...
      <div className="bg-white shadow-2xl rounded-xl w-full max-w-5xl p-8 flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <button
            ...
            className="mt-4 px-6 py-2 bg-gradient-to-r from-purple-500 to-purple-700 text-white rounded-full shadow hover:scale-105 transition"
          >
            Message {name}
          </button>
      ...
      <div className="w-full max-w-5xl mt-12">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">More Professionals</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {accounts.filter(user => user._id !== id).map(user => (
            ...
            >
              ...
              <p
                ...
                className="text-purple-600 font-semibold cursor-pointer hover:underline text-center"
              >
                {user.name}
              </p>
              ...
              <button
                ...
                className="mt-3 px-4 py-1 bg-purple-500 text-white text-xs rounded-full hover:bg-purple-600"
              >
                Message
              </button>
            </div>
          ...
          {accounts.length <= 1 && (
            <p className="text-sm text-gray-500">No other profiles available.</p>
          )}
        </div>
      </div>
    </div>
